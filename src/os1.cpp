...
namespace ouster {
  namespace OS1 {
    ...
    namespace {
      ...
      const std::array<std::pair<lidar_mode, std::string>, 5> lidar_mode_strings = {{{MODE_512x10, "512x10"}, {MODE_512x20, "512x20"}, {MODE_1024x10, "1024x10"}, {MODE_1024x20, "1024x20"}, {MODE_2048x10, "2048x10"}}};
      ...
      int udp_data_socket(int port)
      {
        ...
        for (ai = info_start; ai != NULL; ai = ai->ai_next)
        {
          ...
          // https://github.com/ouster-lidar/ouster_example/pull/81/commits/2e87d99a672966ef2834dbe55f794ce874135242
    ...
    std::string to_string(lidar_mode mode)
    {
      ...
      auto res = std::find_if(
              lidar_mode_strings.begin(), end, [&](const std::pair<lidar_mode, std::string> &p) {
                return p.first == mode;
              });
    ...
    init_client(const std::string &hostname, const std::string &udp_dest_host, lidar_mode mode, int lidar_port,
                int imu_port)
    {
      ...
      success &= do_tcp_cmd(
              sock_fd, {"set_config_param", "udp_port_lidar", std::to_string(lidar_port)}, res);
      success &= res == "set_config_param";

      success &= do_tcp_cmd(
              sock_fd, {"set_config_param", "udp_port_imu", std::to_string(imu_port)}, res);
      success &= res == "set_config_param";
    ...
    bool init_ptp_client(std::shared_ptr<OS1::client>& cli)
    {
      bool success = true;
      std::string res;
      int sock_fd = OS1::cfg_socket(cli->meta["hostname"].asString().c_str());

      success &= OS1::do_tcp_cmd(sock_fd, {"set_config_param", "timestamp_mode", "TIME_FROM_PTP_1588"}, res);
      success &= res == "set_config_param";
